...
export class PlanCompTicketOfferPage {
    ...
    async verifyTabCompTicketOffer() {
        await test.step('User verify with complimentary ticket offer page', async () => {

            const heading = this.locator.headerCompTicket;

            await expect(heading).toBeVisible({ timeout: 10000 });
            await expect(this.page).toHaveURL(/plan-id=9/);
        });
    }
    ...
    async verifyConfirmationCompTicketOffer() {
        await test.step('User verify confirmation plan complimentary ticket offer', async () => {
            ...
            await expect(confirmCompTicket).toBeVisible({ timeout: 10000 });
