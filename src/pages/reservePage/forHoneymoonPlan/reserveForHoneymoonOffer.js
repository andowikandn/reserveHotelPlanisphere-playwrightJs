...
import { ReserveForHoneymoonLocator } from './locatorForHoneymoonOffer.js';

export class PlanForHoneymoonOfferPage {
    constructor(page) {
        this.page = page;
        this.locator = new ReserveForHoneymoonLocator(page);
    }

    async verifyTabForHoneymoonOffer() {
        await test.step('User verify for honeymoon offer page', async () => {

            const heading = this.locator.headerForHoneymoon;

            await expect(heading).toBeVisible();
            await expect(this.page).toHaveURL(/plan-id=8/);
        });
    }
    ...
    async verifyConfirmationForHoneymoonOffer() {
        await test.step('User verify confirmation plan for honeymoon offer', async () => {
            ...
            await expect(confirmForHoneymoon).toBeVisible({ timeout: 10000 });
